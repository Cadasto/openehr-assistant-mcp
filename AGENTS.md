...
- **Resources**:
  ...
  - `Examples` provides `openehr://examples/{kind}/{name}` resources and registers curated example artefacts at startup. Kinds: `aql` (reference AQL queries), `flat` / `structured` (paired simplified-format JSON payloads). Each example is a Markdown file with a metadata header (pattern, demonstrates, related specs/guides) and a fenced code block with the actual query / payload. Search via `examples_search`, fetch via `examples_get`.
