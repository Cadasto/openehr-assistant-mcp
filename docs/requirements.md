...
| **REQ-N9** | Every published MCP tool schema is validated in CI: closed input schemas (`additionalProperties: false`, nullable optional enums, bounded numbers) and output payloads conforming to their declared `outputSchema`. | The SDK validates tool *input* against `inputSchema` but never validates tool *output*, so the output contract has no runtime enforcement — CI is the only thing standing between a schema edit and a broken wire contract. |
