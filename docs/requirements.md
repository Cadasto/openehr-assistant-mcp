...
| **REQ-N10** | A public product website presents the MCP server and the user-facing plugin to end users, and reuses the canonical install documentation rather than restating it; publishing is automated and its output verified before it goes live. | Discoverability and consistent install guidance for clinical integrators. → [ADR-0007](decisions/0007-product-website-mkdocs-github-pages.md) |
