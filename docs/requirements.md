...
| **REQ-N10** | Publish a public product website (MkDocs) for end users covering the MCP server and plugin; reuse canonical install docs without duplication; build locally to `docs-build/` and deploy via GitHub Actions Pages. | Discoverability and consistent install guidance for clinical integrators. → [ADR-0007](decisions/0007-product-website-mkdocs-github-pages.md) |
